    ...
    if (webPageData.value("enabled").toBool()) {
        ...
        for (int i = 0; i < elements.size(); ++i) {
            ...
            if (type == "Text") {
                ...
                code += QString("  html += \"<div class='elem text' style='left:%1px; top:%2px; font-size:%3px; color:%4; font-weight:%5;' id='%6'><span id='val_%6'>%7</span></div>\";\n")
    ...
    if (type == "esp32" || type == "board") {
        ...
        if (hasEvent) {
            code += "// Função executada quando a luminosidade do LDR é alterada (aoAlterar)\n"
                    "// O parâmetro 'luminosidade' contém o valor mapeado (0 a 100%)\n";
            code += emitEventFunction(

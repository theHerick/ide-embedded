...
void MainWindow::buildToolbar() {
    ...
    infoAction->setIcon(style()->standardIcon(QStyle::SP_MessageBoxInformation));
...
void MainWindow::applyTheme() {
    ...
    setStyleSheet(
        ...
        "QToolBar QToolButton { background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #FFFFFF, stop:1 #DCE6F0); border: 1px solid #AEBFCC; border-radius: 9px; color: #0F172A; padding: 7px 11px; font-family: 'Segoe UI', Arial, sans-serif; font-size: 11px; font-weight: 600; min-width: 30px; min-height: 30px; }"
        "QToolBar QToolButton:hover { background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #FFFFFF, stop:1 #D2E2F0); border-color: #8FB2D8; color: #0B1220; }"
        "QToolBar QToolButton:pressed { background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #C9D9E8, stop:1 #EEF5FB); border-color: #7FA2C9; color: #08306B; }"
        "QToolBar QToolButton:checked { background: qlineargradient(x1:0, y1:0, x2:0, y2:1, stop:0 #D9E7F7, stop:1 #BFD3EA); border-color: #7FA2C9; }"
...
void MainWindow::exportLaserPNG() {
    ...
    configDlg.setStyleSheet(
        ...
        "QLabel { color: #475569; font-family: 'Segoe UI', Arial; font-size: 11px; font-weight: 600; }"
        "QLabel#title { color: #0F172A; font-size: 15px; font-weight: 700; }"
        "QLabel#sub { color: #64748B; font-size: 10px; font-weight: 400; }"
        "QDoubleSpinBox { background: #FFFFFF; border: 1px solid #E6EEF3; border-radius: 8px;"
        "  color: #0F172A; padding: 9px 12px; font-size: 13px; font-weight: 600; }"
        "QDoubleSpinBox:focus { border-color: #93C5FD; }"
        "QPushButton { background: #2563EB; border: none; border-radius: 8px; color: #fff;"
        "  padding: 11px 22px; font-weight: 700; font-size: 13px; }"
        "QPushButton:hover { background: #1E40AF; }"
        "QPushButton#cancel { background: rgba(15, 23, 42, 0.04); border: 1px solid #E6EEF3; color: #475569; }"
        "QPushButton#cancel:hover { background: rgba(15, 23, 42, 0.06); color: #0F172A; }"
    );
    ...
    previewDlg.setStyleSheet(
        ...
        "QLabel#meta { color: #64748B; font-size: 11px; font-family: 'Segoe UI', Arial; }"
        "QLabel#badge { background: rgba(15, 23, 42, 0.04); border: 1px solid #E6EEF3; border-radius: 6px;"
        "  color: #475569; font-size: 11px; font-family: 'Segoe UI', Arial;"
        "  padding: 4px 10px; }"
        "QPushButton { background: #2563EB; border: none; border-radius: 8px;"
        "  color: #fff; padding: 11px 24px; font-weight: 700; font-size: 13px;"
...
void MainWindow::showFirmwareInfo() {
    ...
    if (appName.isEmpty()) appName = "IDE Embedded";
    QString version = QCoreApplication::applicationVersion();
    if (version.isEmpty()) version = "(versão não informada)";
    QString projectPath = m_currentProjectPath.isEmpty() ? "(não salvo)" : m_currentProjectPath;

    QString html = QString(
        "<h3>%1</h3>"
        "<p><b>Versão:</b> %2</p>"
        "<p><b>O que é:</b> Uma IDE visual para modelagem de circuitos embarcados, geração de código C++/Arduino e simulação interativa do hardware.</p>"
        "<p><b>Como usar (passos rápidos):</b></p>"
        "<ol>"
        "<li>Adicione componentes pela caixa de ferramentas à esquerda.</li>"
        "<li>Conecte-os com cabos e ajuste propriedades de cada componente.</li>"
        "<li>Use 'Modelar' para criar componentes personalizados quando necessário.</li>"
        "<li>Clique em 'Build' para gerar e validar o código.</li>"
        "<li>Após um Build bem-sucedido, pressione Play para iniciar a simulação.</li>"
        "</ol>"
        "<p><b>Dicas:</b> Salve frequentemente; use 'Visualizar C' para inspecionar o código gerado; acompanhe o console de logs para erros e avisos.</p>"
        "<p><b>Projeto atual:</b> %3</p>"
    ).arg(appName, version, projectPath);

    QMessageBox msg(this);
    msg.setWindowTitle("Sobre o Software");

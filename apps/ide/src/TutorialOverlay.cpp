...
void TutorialOverlay::setSteps(const QVector<TutorialStep>& steps) {
    m_steps = steps;
    m_currentStep = 0;
}
...
int TutorialOverlay::currentStep() const {
    return m_currentStep;
}

qreal TutorialOverlay::pulseRadius() const {
    return m_pulse;
}

void TutorialOverlay::setPulseRadius(qreal r) {
    m_pulse = r;
    update();
}

void TutorialOverlay::addVariableDragStep(int stepIndex, const QString& varKeyword, const QString& targetField) {
    m_dragStepKeywords[stepIndex] = varKeyword;
    m_dragStepTargetFields[stepIndex] = targetField;
}

void TutorialOverlay::clearVariableDragSteps() {
...
void TutorialOverlay::mouseReleaseEvent(QMouseEvent* event) {
    event->accept();
}

void TutorialOverlay::mouseMoveEvent(QMouseEvent* event) {
    event->accept();
}

void TutorialOverlay::mouseDoubleClickEvent(QMouseEvent* event) {
    event->accept();
}

void TutorialOverlay::contextMenuEvent(QContextMenuEvent* event) {
    event->accept();
}

void TutorialOverlay::showStep(int idx) {
...
QWidget* TutorialOverlay::resolveVariableWidget(const QString& keyword) const {
...
QWidget* TutorialOverlay::resolveTargetWidget(const TutorialStep& step) const {
    return step.targetWidget;
}

QRect TutorialOverlay::getTargetRect(const TutorialStep& step) const {
...
void TutorialOverlay::positionCard(const TutorialStep& step) {
...
void TutorialOverlay::drawArrow(QPainter& p, const QRect& target) {
...
QWidget* TutorialOverlay::resolveActionParamWidget() const {
    if (parentWidget()) {
...
QWidget* TutorialOverlay::resolveActionTargetWidget() const {
    if (parentWidget()) {
        QWidget* lastResort = nullptr;
        for (auto* item : parentWidget()->findChildren<QLineEdit*>()) {
...
QWidget* TutorialOverlay::resolveActionCmdWidget() const {
    if (parentWidget()) {
        QWidget* lastResort = nullptr;
